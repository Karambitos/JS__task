    ...
    (function (arrOfTasks) {
      ...
      function renderInProgressTasks(taskList) {
        Object.values(taskList).forEach((task) => {
          if (task.completed) {
            const id = task._id;
            const doneTask = listConteiner.querySelector(
              `[data-task-id="${id}"]`
            );
      ...
      function renoveDisplayNone(taskList) {
      ...
      function deleteTask(id) {
        const isConfirm = confirm(
          `Are you sure, you want to delete "${objOfTasks[id].title}" ?`
        );
      ...
      // create <div>"Task list is empty"</div>
      function createDiv() {
      ...
      function onClickStatus(e) {
        ...
        if (target.classList.contains("progress-btn" && "btn-success")) {
          ...
          target.classList.contains("progress-btn" && "btn-secondary")
        ) {
