...
document.addEventListener("DOMContentLoaded", () => {
    {
        ...
        let newUserArr = [];
        filterTypes(map, users);

        function filterTypes(filter, arr) {
            let result = {};
            for (var i = 0; i < arr.length; i++) {
                ...
                for (let key in user) {
                    if (filter.indexOf(key) > -1)
                        result[key] = user[key];
                    // console.log(filter.indexOf(key))
                }
                newUserArr.push(result);
            }
            return result;
        }
        console.log("1. ", newUserArr);
        ...
        const person = {
            ...
            lastUpdate: '',
            get firstName() {
                this.lastGet = curentData;
                return `${this.name} ${this.lastGet}`
            },
            set firstName(value) {
                this.lastUpdate = curentData;
                this.name = value;
        ...
        let d = new Date();
        let curentData = formatDate(d);

        console.log("4. ", person.firstName, person)
        ...
        const product = {
            ...
            set phone(value) {
                let br = value.split(' ', 1);
                let md = value.replace(br, '').slice(1, );
                this.brand = String(br);
                this.model = String(md);
            },
            price: '$300'
        };
        console.log("5. ", product.phone, product)
        product.phone = "Samsung S8 Gold"
        console.log("5. ", product.phone, product)
